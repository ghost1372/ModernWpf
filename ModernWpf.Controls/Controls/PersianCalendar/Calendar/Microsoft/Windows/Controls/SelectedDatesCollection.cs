...
namespace Microsoft.Windows.Controls
{
    ...
    public sealed class SelectedDatesCollection : ObservableCollection<DateTime>
    {
        ...
        private readonly ModernWpf.PersianCalendar _owner;
        ...
        public SelectedDatesCollection(ModernWpf.PersianCalendar owner)
        {
        ...
        protected override void InsertItem(int index, DateTime item)
        {
            ...
            if (!Contains(item))
            {
                ...
                if (ModernWpf.PersianCalendar.IsValidDateSelection(_owner, item))
                {
        ...
        protected override void SetItem(int index, DateTime item)
        {
            ...
            if (!Contains(item))
            {
                ...
                {
                    if (item != null && DateTime.Compare(this[index], item) != 0 && ModernWpf.PersianCalendar.IsValidDateSelection(_owner, item))
                    {
                        removedItems.Add(this[index]);
                        base.SetItem(index, item);
                        UpdateMinMax(item);
        ...
        internal void AddRangeInternal(DateTime start, DateTime end)
        {
            ...
            foreach (DateTime current in GetDaysInRange(start, end))
            {
                if (ModernWpf.PersianCalendar.IsValidDateSelection(_owner, current))
                {
                    Add(current);
        ...
        {
            if (ModernWpf.PersianCalendar.IsValidDateSelection(_owner, date))
            {
        ...
        {
            _owner.OnSelectedDatesCollectionChanged(new CalendarSelectionChangedEventArgs(ModernWpf.PersianCalendar.SelectedDatesChangedEvent, removedItems, addedItems));
        }
